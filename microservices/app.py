...
# # for getting the precautions we match with the predicted disease
...
def helper(dis):
    ...
    # flattebn used to convert 2d into one d
    pre = [col for col in pre.values.flatten().tolist()]
...
def predict():
    if request.method == "POST":
        ...
        desc, prec ,med, workout , diet = helper(predicted_disease)
            
        data = {
            "description": desc.tolist() if isinstance(desc, pd.Series) else desc,
            "precaution": prec.tolist() if isinstance(prec, pd.Series) else prec,
            "workout": workout.tolist() if isinstance(workout, pd.Series) else workout,
            "diet": diet.tolist() if isinstance(diet, pd.Series) else diet,
            "medication": med.tolist() if isinstance(med, pd.Series) else med
        }


        print(type(desc), type(prec), type(workout), type(diet), type(med))
